...
public class InstallLauncherFile {
    ...
    public static void checkLauncherFiles(SplashActivity activity){
        ...
        //检查Java运行时，Java可能内置在启动器，因此单独处理
        localInstallJava8(activity, progressCallback);
        localInstallJava17(activity, progressCallback);
        //checkJava17(activity);
    }

    @SuppressLint("SetTextI18n")
    public static void localInstallJava8(SplashActivity activity, AssetsUtils.ProgressCallback callback){
        activity.runOnUiThread(() -> {
    ...
    @SuppressLint("SetTextI18n")
    public static void localInstallJava17(SplashActivity activity, AssetsUtils.ProgressCallback callback){
        activity.runOnUiThread(() -> {
            activity.loadingText.setText(activity.getString(R.string.local_installing_Java_17));
        });
        if (!new File(AppManifest.JAVA_DIR + "/JRE17").exists() || !new File(AppManifest.JAVA_DIR + "/JRE17/version").exists() || Integer.parseInt(Objects.requireNonNull(FileStringUtils.getStringFromFile(AppManifest.JAVA_DIR + "/JRE17/version"))) < AppInfo.JAVA_17_VERSION_CODE) {
            ...
            //安装完毕后再次检查，并将安装包内游戏资源释放到本地，最后进入启动器
            if(new File(AppManifest.JAVA_DIR + "/JRE17").exists() && new File(AppManifest.JAVA_DIR + "/JRE17/version").exists()){
                localInstallMinecraftResources(activity,callback);
                enterLauncher(activity);
            }else{
                localInstallJava17(activity,callback);
            }
        }else {
            //检查完成，进入启动器
            activity.runOnUiThread(() -> {
    ...
    public static void localInstallMinecraftResources(SplashActivity activity, AssetsUtils.ProgressCallback callback){
        ...
        //释放游戏资源
        AssetsUtils.getInstance(activity).setProgressCallback(callback).copyOnMainThread("game_resources_directory",AppManifest.DEFAULT_GAME_DIR);
